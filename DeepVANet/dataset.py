...
class DEAP(data.Dataset):
    ...
    def __init__(self, modal='facebio', subject=1, k=1, kind='all', indices=list(range(2400)),label='valence'):
        ...
        if kind == 'train':
            self.indices = indices[: int(0.64 * self.size)]
        if kind == 'val':
            self.indices = indices[int(0.64 * self.size): int(0.8 * self.size)]
        if kind == 'test':
            self.indices = indices[int(0.8 * self.size):]
        if kind == 'all':
    ...
    def __getitem__(self, i):
        ...
        trial = index // 60 + 1
        # segment = index % 60 + 1
        segment = index % 15 + 1   # since we use 4-second segments, there are 15 segments in each trial
