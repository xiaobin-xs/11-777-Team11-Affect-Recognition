...
def demo():
    ...
    parser.add_argument('--epoch', '-e', default=200, help='The number of epochs in training', type=int)
    ...
    parser.add_argument('--test', default=False, help='If using existing best checkpoint, only for testing', type=bool)
    ...
    k = 1
    train(modal=args.modal, dataset=args.dataset, epoch=args.epoch, lr=args.learn_rate, use_gpu=use_gpu,
          file_name=f'./results/{args.dataset}/{args.modal}/{args.dataset}_{args.modal}_{args.label}_s{args.subject}_k{1}_{args.face_feature_size}_{args.bio_feature_size}/{args.dataset}_{args.modal}_{args.label}_s{args.subject}_k{k}_{args.face_feature_size}_{args.bio_feature_size}',
          batch_size=args.batch_size, subject=args.subject, k=1, l=args.label, indices=indices,
          face_feature_size=args.face_feature_size, bio_feature_size=args.bio_feature_size, pretrain=pretrain, skip_train=args.test)
