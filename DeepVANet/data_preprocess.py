...
def video2frames(dataset='DEAP'):
    ...
    if dataset == 'DEAP':
        ...
        for subject in os.listdir(dataset_path):
            print('[video2frames] Processing subject: ', subject)
            if subject.startswith('.'):
                continue
            sub_path = dataset_path+subject
            for video_file in os.listdir(sub_path):
                print('[video2frames] -- Processing subject: ', subject, ', file, ', video_file)
                ...
                last_count = int(frame_count // 40)
                last_frame_path = des_path + subject + '/' + video_file.split('.')[0] + '/' + video_file.split('.')[0] + '_' + str(last_count) + '.png'
                if os.path.exists(last_frame_path):
                    print('[video2frames] -- Last frame exists, skip', subject, ', file, ', video_file)
                    continue
                ...
                count = 0
                # frame_rate = 10
                frame_rate = 40         # we should get a image every 40 frames, so that 5 images will be 200 frames = 4 seconds
                while (True):
                    ret, frame = video.read()
                    if ret:
                        if (c % frame_rate == 0):
...
def face_detection_alignment_cropping(dataset='DEAP'):
    ...
    for subject in os.listdir(root):
        print('[face_detection_alignment_cropping] Processing subject: ', subject)
        if not os.path.exists(des_path + subject):
            os.mkdir(des_path + subject)
        for trial in os.listdir(root+subject):
            print('[face_detection_alignment_cropping] Processing subject: ', subject, ', trial: ', trial)
...
def trial2segments(dataset='DEAP'):
    ...
    if dataset == 'DEAP':
        ...
        for file in os.listdir(root):
            ...
            print('[trial2segments] Processing subject: ', subject)
            ...
            for experiment in range(40):
                print('[trial2segments] -- Processing subject: ', subject, ', trial, ', experiment)
                ...
                # repeat baseline mean from (40, 128) to (40, 512)
                baseline_mean = np.repeat(baseline_mean, 4, axis=1)
                # segments
                # for i in range(60):
                ...
                for i in range(15):
                    data_seg = data[experiment][:, 384 + i * 512:  384 + (i + 1) * 512]
                    data_seg_removed = data_seg - baseline_mean
                    np.save(f'{des_path}s{sub_id}/{sub_id}_{trial}_{i + 1}.npy', data_seg_removed)
...
def preprocess_demo():
    ...
    print('[video2frames] Pre-processing face data...')
    # video2frames('DEAP')

    print('[face_detection_alignment_cropping] Pre-processing face data...')
    ...
    print('[trial2segments] Pre-processing bio-sensing data...')
    trial2segments('DEAP')
