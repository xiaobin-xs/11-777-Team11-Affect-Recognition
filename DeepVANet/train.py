...
def train(modal, dataset, subject, k, l, epoch, lr, batch_size, file_name, indices, face_feature_size=16, bio_feature_size=64, use_gpu=False, pretrain=True, skip_train=False):
    '''
    ...
    if dataset == 'DEAP':
        ...
        if subject == 0:
            ...
            test_data = DEAPAll(modal=modal, k=k, kind='test', indices=indices, label=l)
        ############## per-subjects ##############
        else:
            ...
            test_data = DEAP(modal=modal, subject=subject, k=k, kind='test', indices=indices, label=l)
        bio_input_size = 40
        peri_input_size = 8
    if dataset == 'MAHNOB':
        ############## inter-subjects  ##############
    ...
    patience_max = 20
    current_patience = patience_max


    if not skip_train:
        for epoch in range(epoch):
            print('Starting Epoch: ' + 'train' + str(epoch) + '...')
            out_put('Starting Epoch: ' + 'train' + str(epoch), file_name)
            pred_label = []
            true_label = []
            ...
            print('- Complete Epoch: ' + 'train' + str(epoch) + '| train accuracy: ' + str(train_accuracy.item()))
            out_put('- Complete Epoch: ' + 'train' + str(epoch) + '| train accuracy: ' + str(train_accuracy.item()), file_name)

            val_accuracy = val(modal, model, val_loader, use_gpu)


            print('- Epoch: ' + 'train' + str(epoch) + '| train loss: ' + str(loss_meter.value()[0]) +
                  '| val accuracy: ' + str(val_accuracy.item()))
    ...
    print('Training All Done! Start Testing...')
    test_accuracy, test_f1 = test(modal, model, test_loader, use_gpu)
    perf = f"test accuracy is {best_accuracy}, F1 {test_f1} in epoch {best_epoch}" + "\n"
    print(perf)
    out_put(perf, file_name)
...
@torch.no_grad()
def test(modal, model, dataloader, use_gpu):
    model.eval()
    if use_gpu:
      device = torch.device('cuda')
    else:
