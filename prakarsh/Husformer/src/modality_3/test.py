...
def KD_train(hyp_params, test_loader, KD_model):
    model = load_model(hyp_params, name=hyp_params.name)
    model.eval()
    loader = test_loader 
    total_loss = 0.0
    ...
    with torch.no_grad():
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(loader):
            ...
            batch_size = m1.size(0)
            net = nn.DataParallel(model) if batch_size > 10 else model
            preds, tgt_emb = net(m1,m2,m3)
            preds, ref_emb = KD_model(m1,m2,m3)
            loss_num_itr = cosine_loss(tgt_emb, ref_emb) + l1_reg(tgt_emb, ref_emb)
            total_loss += criterion(preds, eval_attr).item() * batch_size
            results.append(preds)
            truths.append(eval_attr)     
    avg_loss = total_loss / hyp_params.n_test 
